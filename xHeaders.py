...
# --- পরিবর্তন: 'base_url' যোগ করা হয়েছে ---
def DeLet_Uid(id , Tok, base_url):
    print(f' Done FuckinG > {id} ')
    # --- পরিবর্তন: URL ডাইনামিক করা হয়েছে ---
    ...
    if ResPonse.status_code == 400 and 'BR_FRIEND_NOT_SAME_REGION' in ResPonse.text:
        ...
        # --- পরিবর্তন: আরো বিস্তারিত এরর দেখানোর জন্য ---
        return f'[b][c]Erorr ! Status: {ResPonse.status_code}'

# --- পরিবর্তন: 'base_url' যোগ করা হয়েছে ---
def Add_Friend(id, Tok, base_url):
    print(f'Sending Friend Request To > {id}')
    # --- পরিবর্তন: URL ডাইনামিক করা হয়েছে ---
    ...
    payload_hex = f"08{EnC_Uid(id, Tp='Uid')}"
    data = bytes.fromhex(EnC_AEs(payload_hex))

    ResPonse = requests.post(url, headers=headers, data=data, verify=False)

    if ResPonse.status_code == 200:
        return f'[b][c]Request Sent Successfully To : {xMsGFixinG(id)} !'
    elif 'BR_FRIEND_ALREADY_FRIEND' in ResPonse.text:
        return f'[b][c]Already Friends With : {xMsGFixinG(id)}'
    elif 'BR_FRIEND_NOT_SAME_REGION' in ResPonse.text:
        return f'[b][c]Id : {xMsGFixinG(id)} Not In Same Region !'
    elif 'BR_FRIEND_PLAYER_REQUEST_LIST_FULL' in ResPonse.text:
        return f'[b][c]Target\'s Friend List is Full !'
    else:
        # --- পরিবর্তন: আরো বিস্তারিত এরর দেখানোর জন্য ---
        return f'[b][c]Error Sending Request: {ResPonse.status_code}'
