...
async def LagSquad(K, V, region="default"):
    fields = {
        1: 15,
        2: {
            1: 1124759936
    ...
    # Use region-specific packet types like other functions
    if region.lower() == "ind":
        packet_type = '0514'
    ...
    # Debug information
    # print(f"LagSquad fields: {fields}") # Spammy log
    packet_data = await CrEaTe_ProTo(fields)
    packet_hex = packet_data.hex()
    # print(f"LagSquad packet data: {packet_hex}") # Spammy log
    result = await GeneRaTePk(packet_hex, packet_type, K, V)
    # print(f"LagSquad final packet: {result}") # Spammy log
    return result
...
async def xChLeaDer(TarGeT_Uid , K , V, region):
    fields = {
        1: 10,  # --- field number 10 for "Change Leader"
        2: {
            1: int(TarGeT_Uid) # --- field number 1 for target UID
        }
    }

    # Use region-specific packet types
    if region.lower() == "ind":
        packet = '0514'
    elif region.lower() == "bd":
        packet = "0519"
